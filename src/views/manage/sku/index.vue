      ...
      <el-table-column label="商品类型" align="center" prop="classId" >
        <template #default="scope">  
          <div v-for="item in skuClassList" :key="item.classId">
              <span v-if="scope.row.classId === item.classId">
                {{ item.className }}
              </span>
          </div>
        </template>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="skuRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="商品价格" prop="price">
          <el-input-number :min="0.01" :max="999.99" precision="2" step="0.05" v-model="form.price" placeholder="请输入商品价格" />元
        </el-form-item>
